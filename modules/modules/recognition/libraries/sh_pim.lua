...
if lia.config.FakeNamesEnabled then
    PIM:AddOption(
        "Allow This Player To Recognize You With A Fake Name",
        {
            ...
            onRun = function(client, target) if CLIENT then Derma_StringRequest("Allow this person to recognize you by a fake name.", "Enter a fake name to display to this player.", default or "", function(text) if text then netstream.Start("rgnDirect", target, text) end end) end end
        }
    )
end
----------------------------------------------------------------------------------------------
