...
function PANEL:Init()
    ...
    for _,ply in ipairs(player.GetAll()) do
        if staffGroups[ply:GetUserGroup()] then
    ...
    self.staffListHeader:SetContentAlignment(5)
    self.staffListHeader:SetFont("nutGenericFont")
    self.staffListHeader:SetTextColor(color_white)
    self.staffListHeader:SetExpensiveShadow(1, color_black)
    self.staffListHeader:SetTall(self.staffListHeader:GetTall() + 12)
    self.staffListHeader.Paint = function(this, w, h)
        surface.SetDrawColor(127, 0, 4, 50)
        surface.DrawRect(0, 0, w, h)
    end
...
function PANEL:Think()
    if ((self.nextUpdate or 0) < CurTime()) then
        ...
        for k, v in ipairs(self.teams) do
            ...
            if( visible and k == FACTION_HIDDEN ) then
                v:SetVisible( false );
            ...
            if (visible and amount == 0) then
                v:SetVisible(false)
                self.layout:InvalidateLayout()
            elseif (!visible and amount > 0 and k != FACTION_HIDDEN ) then
                v:SetVisible(true)
            end 
        end
...
function PANEL:addPlayer(client, parent)
    ...
    slot.name:Dock(TOP)
    slot.name:DockMargin(65, 7, 48, 0)
    ...
    slot.desc:Dock(FILL)
    slot.desc:DockMargin(65, 1, 48, 0)
    ...
    function slot:update()
        ...
        if (offDutySB[LocalPlayer():GetUserGroup()] or (LocalPlayer() == client) or LocalPlayer():Team() == FACTION_HIDDEN) then
