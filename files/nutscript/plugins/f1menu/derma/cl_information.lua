local PANEL = {}
	function PANEL:Init()
		...
		self:SetSize(ScrW() * 0.75, ScrH() * 0.7)
		self:Center()
		...
		if (!suppress or (suppress and !suppress.all)) then
			...
			if (!suppress or !suppress.info) then
				...
				self.info:DockMargin(125, ScrH() * 0.16, 0, 0)
			end

            surface.CreateFont("F1CharacterNameLabel", {
                font = "Montserrat",
                size = 20,
                weight = 500,
            })

            surface.CreateFont("F1CharacterName", {
                font = "Montserrat",
                size = 42,
                weight = 800,
            })
            ...
            if (!suppress or !suppress.name) then
                self.charactername = self.info:Add("DLabel")
                self.charactername:SetFont("F1CharacterNameLabel")
                self.charactername:SetText("Character Name")
                self.charactername:SetTall(20)
				self.charactername:Dock(TOP)
                self.charactername:SetZPos(1)
                self.charactername:DockMargin(0, 0, 0, 0)
				self.charactername:SetTextColor(color_white)
				self.charactername:SetExpensiveShadow(1, Color(0, 0, 0, 150))
            end


			if (!suppress or !suppress.name) then
				self.name = self.info:Add("DLabel")
				self.name:SetFont("F1CharacterName")
            ...
            if (!suppress or !suppress.desc) then
                self.descLabel = self.info:Add("DLabel")
                self.descLabel:SetFont("F1CharacterNameLabel")
                self.descLabel:SetText("Character Description")
                self.descLabel:SetTall(20)
				self.descLabel:Dock(TOP)
                self.descLabel:SetZPos(3)
                self.descLabel:DockMargin(0, 30, 0, 10)
				self.descLabel:SetTextColor(color_white)
				self.descLabel:SetExpensiveShadow(1, Color(0, 0, 0, 150))
            end
