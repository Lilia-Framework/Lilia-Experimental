...
if (CLIENT) then
	...
	function PLUGIN:CalcView(client, origin, angles, fov)
		...
		if (client:CanOverrideView() and LocalPlayer():GetViewEntity() == LocalPlayer()) then
			...
			if (NUT_CVAR_TP_CLASSIC:GetBool() or (owner:KeyDown(bit.bor(IN_FORWARD, IN_BACK, IN_MOVELEFT, IN_MOVERIGHT)) and owner:GetVelocity():Length() >= 10) ) then
    ...
    hook.Add( "Think", "ThirdPerson_Toggle_NN", function()
        if not IsValid(LocalPlayer()) then return end
        if nut.gui.chat.active then return end
        if gui.IsGameUIVisible() then return end
        local cur_time = CurTime()

        if (inputDelay or 0) > cur_time then return end

        inputDelay = cur_time + 0.25

        if input.IsKeyDown( KEY_F4 ) then
            LocalPlayer():ConCommand("nut_tp_toggle")
        end
    end )
end