...
function GM:CanTool(client, _, tool)
    ...
    if IsValid(client) and (client:getChar():hasFlags("t") or client:isStaffOnDuty()) and CAMI.PlayerHasAccess(client, privilege, nil) then
        ...
        if tool == "remover" then
            if table.HasValue(MODULE.RemoverBlockedEntities, entity:GetClass()) and validEntity then return CAMI.PlayerHasAccess(client, "Staff Permissions - Can Remove Blocked Entities", nil) end
