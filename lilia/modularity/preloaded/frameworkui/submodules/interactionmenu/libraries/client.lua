...
------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
function MODULE:OpenMODULE()
    if IsValid(MODULE_Frame) then MODULE_Frame:Close() end
    ...
    timer.Remove("MODULE_Frame_Timer")
    timer.Create("MODULE_Frame_Timer", 30, 1, function() if frame and IsValid(frame) then frame:Close() end end)
    ...
    for name, opt in pairs(self.options) do
        if opt.shouldShow(LocalPlayer(), traceEnt) and traceEnt:IsPlayer() and self:CheckDistance(LocalPlayer(), traceEnt) then
            ...
            function p:DoClick()
                ...
                if opt.runServer then netstream.Start("MODULERunOption", name) end
            end
        end
    end
