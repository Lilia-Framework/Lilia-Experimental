...
function MODULE:IsRecognizedChatType(chatType)
    return table.HasValue(MODULE.ChatIsRecognized, chatType)
end
...
function MODULE:ShouldAllowScoreboardOverride(client, var)
    local character = client:getChar()
    local ourCharacter = LocalPlayer():getChar()
    if MODULE.RecognitionEnabled and table.HasValue(MODULE.ScoreboardHiddenVars, var) and (client ~= LocalPlayer()) and not (ourCharacter:doesRecognize(character:getID()) and ourCharacter:doesFakeRecognize(character:getID())) then return true end
end
