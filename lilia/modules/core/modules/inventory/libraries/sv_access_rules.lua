...
function MODULE:ItemCombine(client, item, target)
    if target.onCombine then if target:call("onCombine", client, nil, item) then return end end
    if item.onCombineTo then if item and item:call("onCombineTo", client, nil, target) then return end end
end
